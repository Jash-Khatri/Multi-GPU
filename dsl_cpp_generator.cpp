...
void dsl_cpp_generator :: addCudaKernel(forallStmt* forAll)
{
  ...
  header.pushString("(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta, bool *d_modified_next, int *triangle_count, int dnum, int perdevicevertices, int devicecount, int minid");
  ...
  sprintf(strBuffer, "if( v < (dnum*perdevicevertices - dnum*minid ) && v >= ( (dnum == devicecount-1) ? (V - dnum*minid) : ( (dnum+1)*perdevicevertices ) ) ) return;");
  header.pushstr_newL(strBuffer);
...
void dsl_cpp_generator::generateForAll(forallStmt* forAll, bool isMainFile) {
  ...
  if (forAll->isForall()) { // IS FORALL
    ...
    */
    ...
    main.pushString(">>>");
    main.push('(');
    main.pushString("V,E,d_meta[i],d_data[i],d_src,d_weight,d_rev_meta,d_modified_next, triangle_count[i], i, perdevicevertices, devicecount, minid");
    //  if(currentFunc->getParamList().size()!=0)
     // main.pushString(",");
      for(Identifier* iden: vars)
      {
...
void dsl_cpp_generator::generateFuncBody(Function* proc, bool isMainFile) {
  ...
  if (genCSR) {
    ...
    sprintf(strBuffer, "int* d_meta[devicecount];");
    main.pushstr_newL(strBuffer);
    sprintf(strBuffer, "int* d_data[devicecount];");
    main.pushstr_newL(strBuffer);
  ...
  // Below two lines are same for every multi-GPU backend hence Hardcoded..
     ...
     main.pushstr_newL("int minid = V+1;");

  main.NewLine();
     main.pushstr_newL("for( int id = perdevicevertices ; id < V ; id++ )");
     main.pushstr_newL("{");
     main.pushstr_newL("for (int edge = h_meta[id]; edge < h_meta[id+1]; edge++) ");
     main.pushstr_newL("{");
     main.pushstr_newL("minid = min(minid, h_data[edge] );");
     main.pushstr_newL("}");
     main.pushstr_newL("}");
     main.pushstr_newL("minid = min(minid,perdevicevertices);	");

  //main.pushstr_newL("h_meta = (int *)malloc( (V+1)*sizeof(int));");
  main.NewLine();
     main.pushstr_newL("for(int i=0;i<devicecount;i++)");
     main.pushstr_newL("{");
     main.pushstr_newL("if( i == 0 ) ");
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMalloc( &d_meta[i], sizeof(int) * ( 1 + perdevicevertices ) );");
     main.pushstr_newL("}");
     main.pushstr_newL("else ");
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMalloc( &d_meta[i], sizeof(int) * ( 1 + (V - minid) ) ) ;");
     main.pushstr_newL("}");
     main.pushstr_newL("}");
  ...
  main.NewLine();
     ...
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMalloc( &d_data[i], sizeof(int) * (  h_meta[(i+1)*perdevicevertices] - h_meta[i*perdevicevertices]  ) );");
     main.pushstr_newL("}");
     main.pushstr_newL("else ");
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMalloc( &d_data[i], sizeof(int) * (  h_meta[V] - h_meta[minid] ) ); ");
     main.pushstr_newL("}");
     main.pushstr_newL("}");
  ...
  main.NewLine();
     ...
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMalloc( &triangle_count[i], (1)*sizeof(int) );");
     main.pushstr_newL("}");

  main.NewLine();

     main.pushstr_newL("int *newOA;");
     main.pushstr_newL("newOA = (int *)malloc( (V+1)*sizeof(int));");
     main.pushstr_newL("long long int count_indices = 0;");
     main.pushstr_newL("for(int i=0;i<V+1;i++)");
     main.pushstr_newL("{");
     main.pushstr_newL("if(i > 0)");
     main.pushstr_newL("count_indices += h_meta[i] - h_meta[i-1];");
     main.pushstr_newL("else ");
     main.pushstr_newL("count_indices = 0;");
     main.pushstr_newL("newOA[i] = count_indices;");
     main.pushstr_newL("if( i == minid ) ");
     main.pushstr_newL("count_indices = 0;");
     main.pushstr_newL("}");  
    ...
    main.NewLine();
     ...
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMemcpy(d_meta[i], h_meta , sizeof(int) * (1+perdevicevertices), cudaMemcpyHostToDevice);");
     main.pushstr_newL("}");
     main.pushstr_newL("else ");
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMemcpy(d_meta[i], newOA+minid, sizeof(int) * ( 1 + (V - minid) ), cudaMemcpyHostToDevice);");
     main.pushstr_newL("}");
     main.pushstr_newL("}");
     ...
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMemset(d_meta[i], 0, sizeof(int) );");
     main.pushstr_newL("}");

    main.NewLine();

     main.pushstr_newL("int *newedgeList;");
     main.pushstr_newL("newedgeList = (int *)malloc( (E)*sizeof(int));");

	main.NewLine();

     main.pushstr_newL("for(int i=0;i<E;i++)");
     main.pushstr_newL("{");
     main.pushstr_newL("newedgeList[i] = h_data[i];");
     main.pushstr_newL("}");
	
	main.NewLine();

     main.pushstr_newL("for(int j=h_meta[minid];j<h_meta[V];j++) ");
     main.pushstr_newL("{");
     main.pushstr_newL("newedgeList[j] -= (minid);");
     main.pushstr_newL("}");

	main.NewLine();

	main.pushstr_newL("long long int sum_of_edges = 0;");
     main.pushstr_newL("for(int i=0;i<devicecount;i++)");
     main.pushstr_newL("{");
     main.pushstr_newL("if( i == 0 ) ");
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMemcpy(d_data[i], h_data + sum_of_edges, sizeof(int) * (  h_meta[(i+1)*perdevicevertices] - h_meta[i*perdevicevertices]  ), cudaMemcpyHostToDevice);");
     main.pushstr_newL("sum_of_edges += (  h_meta[(i+1)*perdevicevertices] - h_meta[i*perdevicevertices] );");
     main.pushstr_newL("}");
     main.pushstr_newL("else ");
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMemcpy(d_data[i], newedgeList + h_meta[minid] , sizeof(int) * ( h_meta[V] - h_meta[minid] ), cudaMemcpyHostToDevice);");
     main.pushstr_newL("}");
     main.pushstr_newL("}");
	...
	main.NewLine();
     ...
     main.pushstr_newL("{");
     main.pushstr_newL("cudaSetDevice(i);");
     main.pushstr_newL("cudaMemset(triangle_count[i], 0, sizeof(int) );");
     main.pushstr_newL("}");
